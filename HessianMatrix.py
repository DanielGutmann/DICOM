...
class HessianMatrix(object):
    ...
    def HessianValues(self, Image3D, keypoints):
        img_dx = np.diff(Image3D, axis=0)
        img_dy = np.diff(Image3D, axis=1)
        img_dz = np.diff(Image3D, axis=2)

        img_dxx = np.diff(img_dx, axis=0)
        img_dxy = np.diff(img_dx, axis=1)
        img_dxz = np.diff(img_dx, axis=2)

        img_dyx = np.diff(img_dy, axis=0)
        img_dyy = np.diff(img_dy, axis=1)
        img_dyz = np.diff(img_dy, axis=2)

        img_dzx = np.diff(img_dz, axis=0)
        img_dzy = np.diff(img_dz, axis=1)
        img_dzz = np.diff(img_dz, axis=2)
